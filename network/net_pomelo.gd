...
func _process(_delta):
	...
	if(status != socket.STATUS_CONNECTED):
		return
	if isHandshaked and ((Time.get_unix_time_from_system() - lastServerTick) > (heartbeatInterval*2+gapThreshold)):
...
func _sendMessage(reqid,route,msg):
	print("send data ",reqid,route,msg)
	if useCrypto:
...
func _send(msg):
	...
	print('发送 ',bytedata.get_string_from_utf8(), '结果',rets)
