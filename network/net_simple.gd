...
# 发送数据
func SendData(data: PackedByteArray, msgName: String = "", node: int = GT_NONE) -> bool:
	if connHandle.get_status() != connHandle.STATUS_CONNECTED:
		#start_reconnect()
		return false
	if msgName == "":
	...
	if msgTable.has(msgName):
		send_data(msgTable[msgName], data, node)
	return true
...
func start_reconnect():
	print("正在连接服务器")
	connHandle.disconnect_from_host()
	var err = connHandle.connect_to_host(serverIP, serverPort)
	if err == OK:
		print("开始连接")
		connHandle.poll()
	else:
...
func heartbeat():
	semaphore.post()
	SendData(loginProto.PingReq.new().to_bytes(), "PingReq", GT_PING)
...
# 发送数据
func send_data(packet_id: Variant, data: PackedByteArray, node = GT_LOGIN):
	# 将数据转换为字节格式
	var id = int(packet_id)
	# 获取数据内容的长度
	var size = 4 + data.size()
	var data_length = size
	var packet = PackedByteArray()

	packet.append(node >> 32 & 0xff)
	packet.append(node >> 16 & 0xff)
	packet.append(node >> 8 & 0xff)
	packet.append(node & 0xff)

	packet.append(data_length >> 32 & 0xff)
	packet.append(data_length >> 16 & 0xff)
	packet.append(data_length >> 8 & 0xff)
	packet.append(data_length & 0xff)

	packet.append(id >> 32 & 0xff)
	packet.append(id >> 16 & 0xff)
...
func recv_data():
	...
	if revNum > 0:
		var node = connHandle.get_u32()
		var revlen = connHandle.get_u32()
		var recvData = connHandle.get_data(revlen)
		if recvData[0] == OK and 0 < revlen:
			print("接收到:", node, " revlen:", revlen, " recvData:", recvData[1], "revNum:", revNum)
			var index: int = 0
			var data: Array = recvData[1]
			var msgLen = data.size()
			var msgId: int = data[index] * 256 + data[index + 1]
			if msgId > 0:  #过滤心跳打印
				print("收到的msgID:", msgId, " 消息长度:", msgLen)
			# 消息处理，即信号派发
			handleMsg(msgId, data.slice(2))
		else:
